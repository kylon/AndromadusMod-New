...
int vision_wifi_get_mac_addr(unsigned char *buf);
...
#if defined(HW_OOB) || defined(CONFIG_BCMDHD_GOOGLE)
...
static struct wifi_platform_data vision_wifi_control = {
	...
	.get_mac_addr	= vision_wifi_get_mac_addr,
...
int vision_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk(KERN_INFO"vision_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
